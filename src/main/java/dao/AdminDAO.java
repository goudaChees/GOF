...
//	관리자 멤버 페이지 출력 메서드
		...
		try(
			...
			try (
				...
				while (rs.next()) {					
					...
					b1dto.setItem_price(rs.getLong("item_price"));
	...
	public List<Board1_ReplyDTO> selectBrd1ReplyPage (int page) throws Exception {
		...
		String sql = "select * from  (select row_number() over(order by seq desc) line,"
	...
	public List<Board1_ReplyDTO> brd1ReplySearch (String toSearch, int searchCategory, int page) throws Exception {
		
		String sql = "";
		if (searchCategory == 1) {
			sql = "select * from (select row_number() over(order by seq desc) line,"
					+ " board1_reply.* from board1_reply where writer like ?) where line between ? and ?";
		}else if (searchCategory == 2) {
			sql = "select * from (select  row_number() over(order by seq desc) line,"
					+ " board1_reply.* from board1_reply where contents like ?) where line between ? and ?";
		}else {
			sql = "select * from (select row_number() over(order by seq desc) line,"
					+ " board1_reply.* from board1_reply) where line between ? and ?";
		}
	...
	public List<Board2_replyDTO> brd2ReplySearch (String toSearch, int searchCategory, int page) throws Exception {
		String sql = "";
		
		if (searchCategory == 1) {
			sql = "select * from (select row_number() over(order by seq desc) line,"
					+ " board2_reply.* from board2_reply where writer like ?) where line between ? and ?";
		}else if (searchCategory == 2) {
			sql = "select * from (select  row_number() over(order by seq desc) line,"
					+ " board2_reply.* from board2_reply where contents like ?) where line between ? and ?";
		}else {
			sql = "select * from (select row_number() over(order by seq desc) line,"
					+ " board2_reply.* from board2_reply) where line between ? and ?";
		}
	...
	public int getBrdReplySearchTotalCount (int searchCategory, int boardNum, String toSearch) throws Exception {
		String sql = "";
		int result = 0;
		
		if (searchCategory == 1)  {
			sql = "select count(*) from board"+boardNum+" where writer like ?";
		}else if (searchCategory == 2) {
			sql = "select count(*) from board"+boardNum+" where contents like ?";
		}
	...
	public String getBrdReplySearchPageNavi (int currentPage, int searchCategory, int boardNum, String toSearch) throws Exception {
		int recordTotalCount = this.getBrdReplySearchTotalCount(searchCategory, boardNum, toSearch); 
